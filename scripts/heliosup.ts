...
class AndroidHeliosFactory extends HeliosFactory {
  ...
  protected getLibrarySource(): readonly string[] {
    return [
      ...
      '      client.start().await;',
      '',
      '      info!("block number one is {}", client.get_block_number().await.unwrap().to_string());',
      '      thread::sleep(Duration::from_millis(10000));',
      '      info!("block number two is {}", client.get_block_number().await.unwrap().to_string());',
      '      thread::sleep(Duration::from_millis(10000));',
      '      info!("block number thr is {}", client.get_block_number().await.unwrap().to_string());',
      '      thread::sleep(Duration::from_millis(10000));',
      '',
      '      self.client = Some(client);',
      '    });',
